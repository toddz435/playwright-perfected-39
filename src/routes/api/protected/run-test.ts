      ...
      POST: async ({ request }) => {
        try {
          ...
          if (test.type === "api") {
            ...
            const result = await runBrowserSteps(steps, { startIdx, heal: healSelector });
          ...
          const { data: run, error: rErr } = await sb.from("runs").insert({
            ...
            duration_ms: duration, steps: stepResults, summary: { type: test.type, total: stepResults.length, passed: stepResults.filter(s => s.status === "passed").length, healed: stepResults.filter(s => s.status === "healed").length, failed: stepResults.filter(s => s.status === "failed").length },
          }).select().single();
